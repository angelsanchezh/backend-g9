...
@app.route('/', methods=['GET'])
def inicio():
    return {'messsage': 'mi primera api de alumnos' }

@app.route('/inicio',methods=['GET'])
def pagina_inicial():
    return render_template('inicio.html')

@app.route('/mostrar_alumnos',methods=['GET'])
def devolver_alumnos():
    cursor=mysql.connection.cursor()
    cursor.execute("select * FROM alumnos")
    resultado = cursor.fetchall()
    ...
    #return{
    #    'mesage':'estos son los alumnos',
     #   'content': resultado_final
      #   }
    return render_template('mostrar-alumnos.html', alumnos=resultado_final, mensaje='hola desde flask')
